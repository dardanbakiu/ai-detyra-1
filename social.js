...
const RANDOM_MUTATIONS = 1
const MAX_DESCENDANTS_TO_EXPLORE = 10

function geneticSolver(groups, ofSize, forRounds, onProgress) {
  ...
  const weights = _.range(totalSize).map(() => _.range(totalSize).fill(0)) // Matrice e fqinjesise per te gjithe lojtaret (Array i array-ve)

  const rounds = []
  const roundScores = []

  for (let round = 0; round < forRounds; round++) {
    let topOptions = _.range(2).map(() => score(generatePermutation(), weights))
    let generation = 0
